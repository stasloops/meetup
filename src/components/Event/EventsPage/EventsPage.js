...
export const EventsPage = observer(() => {
  ...
  const queryParams = params.theme
  ...
  return (
    <>
      ...
      <PageContent>
        {store.eventsStore._events.length !== 0 ? (
          <InfiniteLoader
            o={{ width: '100%' }}
            style={{ marginLeft: '-5px' }}
            dataLength={store.eventsStore._events.length}
            next={() => fetchNextPage()}
            hasMore={store.eventsStore.hasMore}
            loader={<br />}
          >
            <EventGrid>
              {store.eventsStore?._events?.map((event) => (
              ...
              <FetchButton
                style={{
                  visibility:
                    store.eventsStore._fetchConfig.skip + store.eventsStore._fetchConfig.take <= store.eventsStore._count
                      ? ''
                      : 'hidden',
                }}
                backgroundColor
