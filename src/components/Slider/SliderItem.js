    ...
    <SliderContent isNews={isNews} backgroundColorImage={promoBackground}>
      ...
      <SliderLeftColumn>
        <SliderAboutWrapper>
          {isSlideData && (
            <SliderContentTittle>
              {slideData.name.length > MAX_TITTLE_LENGTH
                ? `${slideData.name.slice(0, MAX_TITTLE_LENGTH)}...`
                : slideData.name}
            </SliderContentTittle>
          )}
          <StaticTextBlock>
            {title.length > MAX_TEXT_LENGTH ? `${title.slice(0, MAX_TEXT_LENGTH)}...` : title}
          </StaticTextBlock>
          {!isNews && (
            <SliderEventButtonsBlock>
              {isSlideData && <EventButtonsBlock horizontal hideLike openEvent event={slideData} />}
            </SliderEventButtonsBlock>
          )}
        </SliderAboutWrapper>
      </SliderLeftColumn>
