...
export function SliderBarNew({ slidesData, changeSlide, currSlide }) {
  const slider = useRef();
  ...
  const dataLength = useMemo(() => slidesData.length, [slidesData.length]);
  const usedSlides = useMemo(() => {
    ...
    const k = [];
    const codes = [];
    const cur = currSlide.currentSlide;
    ...
    if (dataLength > viewedSize) {
      if (pos === 0) {
        ...
        const k3 =
          pos + viewedSize < dataLength ? keys.slice(pos + viewedSize, pos + viewedSize + 1) : keys.slice(0, 1);
        k.push(...k1, ...k2, ...k3);
  ...
  }, [currSlide, dataLength, slidesData]);
  ...
  }, [changeSlide, currSlide.currentSlide]);
    ...
    <SliderBarWrapper>
      ...
      <SliderBarItems ref={slider} padding={LEFT_SLIDER_PADDING}>
        {usedSlides.data.map((elem, index) => {
          ...
          return (
            <SliderBarItem
              key={usedSlides.codes[index]}
              imageSVG={elem?.promoImage?.imageS || elem?.imageSVG}
              imgSrc={elem?.promoImage?.imageS || null}
              current={currSlide.currentSlide % dataLength === usedSlides.idx[index]}
              setCurrentSlide={() => currSlide.setCurrentSlide(usedSlides.idx[index])}
