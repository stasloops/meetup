...
export function EventMap({ event }) {
  const latitude = Number(
    event?.eventLocation?.eventPlatform?.latitude || event?.eventLocation?.eventGeolocation?.latitude,
  );
  const longitude = Number(
    event?.eventLocation?.eventPlatform?.logitude || event?.eventLocation?.eventGeolocation?.longitude,
  );
