...
export const FilterForm = observer(({ onChange, initialState, mobileState, countParams, setCountParams }) => {
  const { handleSubmit, control, reset, setValue } = useFormContext({
    values: store.eventsStore.fetchConfig,
  });
  ...
  const organizerOptions = async () => {
    ...
    const sortedOrganizations = organizers.sort((a, b) => {
  ...
  const searchString = store.eventsStore.fetchConfig.name;
  ...
  useLayoutEffect(() => {
    if (eventsStore.inCardOrganizerSelect) {
      const { organizer } = eventsStore.fetchConfig;

      setValue('organizer', { value: organizer, label: organizer });
      eventsStore.toggleInCardOrganizerSelect(false);
      handleSubmit(onSubmit)();
    }
  }, [eventsStore, eventsStore.fetchConfig, eventsStore.inCardOrganizerSelect, handleSubmit, onSubmit, setValue]);
    ...
    <FFWrapper>
      <FF className={mobileState ? 'open' : ''} style={{ paddingTop: 0, paddingRight: 0 }}>
        ...
        <FFSectionNull>
          <AsyncSelect
            defaultValue={
              store.eventsStore.fetchConfig.organizer !== null
                ? {
                    value: store.eventsStore.fetchConfig.organizer,
                    label: store.eventsStore.fetchConfig.organizer,
                  }
                : undefined
            }
        ...
        <FFSectionCheck>
          <FFSection style={{ paddingTop: 0, marginTop: 0 }}>
            <FilterSectionHeader>Тип</FilterSectionHeader>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      ...
                      <FormCheckField
                        {...field}
                        value={
                          etcConfig.fetchConfig.format &&
                          (item.name === etcConfig.fetchConfig.format ||
                            etcConfig.fetchConfig.format.includes(item.name))
                        }
                ...
                return (
                  <Controller
                    ...
                    render={({ field }) => (
                      ...
                      <FormCheckField
                        {...field}
                        defaultValue={etcConfig.fetchConfig.payment && item.id === etcConfig.fetchConfig.payment}
                        label={filterMatrix[item.name]}
                        // eslint-disable-next-line no-underscore-dangle
                        count={item._count}
                      />
          ...
          <FFSection>
            <FilterSectionHeader style={{ marginBottom: 0 }}>Формат</FilterSectionHeader>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <FormCategoryCheckField
                        {...field}
                        {...etcConfig.fetchConfig.type}
                        label={key}
                        count={data.count}
                        value={
                          etcConfig.fetchConfig.type !== null &&
                          ((key === 'Большие' && etcConfig.fetchConfig.type.includes('Конференция')) ||
                            (key === 'Образовательные' && etcConfig.fetchConfig.type.includes('Курс')) ||
                            (key === 'Специализированные' && etcConfig.fetchConfig.type.includes('Семинар')) ||
                            (key === 'Небольшие' && etcConfig.fetchConfig.type.includes('Митап')) ||
                            (key === 'Соревновательные' && etcConfig.fetchConfig.type.includes('Хакатон')))
                        }
